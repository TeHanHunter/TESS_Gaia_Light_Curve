...
def bg_mod(source, q=None, aper_lc=None, psf_lc=None, portion=None, star_num=0, near_edge=False):
    ...
    aper_local_bg = np.nanmedian(aper_lc[q]) - aperture_bar
    if np.isnan(aper_local_bg):
        aper_local_bg = 0
    aper_lc = aper_lc - aper_local_bg
    psf_bar = bar
    psf_local_bg = np.nanmedian(psf_lc[q]) - psf_bar
    if np.isnan(psf_local_bg):
        psf_local_bg = 0
    psf_lc = psf_lc - psf_local_bg
    ...
    if near_edge:
        cal_psf_lc = psf_lc
        return aper_local_bg, psf_local_bg, aper_lc, psf_lc, cal_aper_lc, cal_psf_lc
    else:
