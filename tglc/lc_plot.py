...
def figure_3():
    ...
    tglc_mag = np.load('/home/tehan/Documents/tglc/prior_mad/mag.npy')
    MAD_aper = np.load('/home/tehan/Documents/tglc/prior_mad/MAD_aper.npy')
    MAD_psf = np.load('/home/tehan/Documents/tglc/prior_mad/MAD_psf.npy')
    aper_precision = 1.46 * MAD_aper / (np.sqrt(2) * 1.5e4 * 10 ** ((10 - tglc_mag) / 2.5))
    psf_precision = 1.46 * MAD_psf / (np.sqrt(2) * 1.5e4 * 10 ** ((10 - tglc_mag) / 2.5))
    ...
    runningmed = ndimage.median_filter(ratio, size=20000, mode='nearest')
...
def figure_15():
    ...
    for i in trange(len(files)):
        ...
        if mad[50] < 100:
            ax1.plot(x, mad, color='C1', alpha=0.02)
