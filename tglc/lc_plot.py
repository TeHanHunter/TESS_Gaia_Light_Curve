...
def figure_4_tglc(folder='/Users/tehan/Documents/TGLC/', contamrt_min=0.0):
    ...
    ground = [156648452, 154293917, 271893367, 285048486, 88992642, 454248975, 428787891, 394722182, 395171208,
              ...
              419411415, 281408474, 460984940, 68007716, 39414571, 8599009, 33595516, 458419328, 336128819, 417646390,
              240823272, 147977348, 144700903, 258920431, 280655495, 66561343, 16005254, 375506058, 279947414,
              239816546, 361343239] + [90850770, 97568467, 263179590, 194795551, 139375960, 100389539, 250111245, 268301217, 455784423] + [439366538, 452006073, 23769326, 401125028, 470171739, 166184428, 269333648, 306648160, 58825110, 165464482, 154220877, 388076422, 178709444, 259172249, 69356857, 119585136]
    ...
    ax[0].vlines(0, ymin=0, ymax=50, color='k', ls='dashed', lw=1, zorder=3)
    ax[0].set_xlabel('')
    ax[0].set_ylabel('Error Weighted Counts')
    ax[0].legend(loc='upper right')
    # ax[0].set_xticks([-0.06, -0.04, -0.02, 0, 0.02, 0.04, 0.06],
    ...
    no_ground = [428699140, 157698565, 119584412, 262530407, 219854185, 140691463, 237922465,
                 ...
                 264678534, 271971130, 204650483, 394918211, 321857016, 290348383, 436873727, 362249359, 372172128] + [
                    ...
                    407126408, 55650590, 335630746, 55525572, 342642208, 394357918] + [52005579, 219850915, 39699648, 232976128, 393818343, 198241702, 154872375, 224297258, 158002130, 198356533, 391949880, 232982558, 306955329, 237232044, 219857012, 417931607, 289580577, 268334473, 21535395, 287080092, 142381532, 9348006, 286916251, 445903569, 8967242, 441765914, 166648874, 142387023, 201177276, 91987762, 1167538, 159781361, 417931300, 389900760, 391903064, 8348911, 153065527, 382602147, 268532343, 22233480, 120826158]
...
def rogers_2023_rocky_core(mass):
    ...
    r1 = a1[0] + a1[1] * np.log(mass) + a1[2] * np.log(mass) ** 2 + a1[3] * np.log(mass) ** 3 + a1[4] * np.log(mass) ** 4
    r2 = a2[0] + a2[1] * np.log(mass) + a2[2] * np.log(mass) ** 2 + a2[3] * np.log(mass) ** 3 + a2[4] * np.log(mass) ** 4
    rho = mass / r ** 3
    rho1 = mass / r1 ** 3
    rho2 = mass / r2 ** 3
...
def figure_9(folder='/Users/tehan/Documents/TGLC/', recalculate=False):
    ...
    no_ground = [428699140, 157698565, 119584412, 262530407, 219854185, 140691463, 237922465,
                 ...
                 219854519, 333657795, 200322593, 287256467, 206541859, 420112589, 261867566, 10837041, 70513361,
                 148673433, 229510866, 321669174, 183120439, 149845414, 293954617, 256722647, 280206394, 468574941,
                 29960110, 141488193, 392476080, 158588995, 49428710, 410214986, 441738827, 220479565,
                 172370679, 116483514, 350153977, 37770169, 212957629, 393831507, 207110080, 190496853,
                 404505029, 207141131, 439456714, 394137592, 267263253, 192790476, 300038935, 169249234, 159873822,
                 394561119, 142394656, 318753380, 422756130, 339672028, 176956893, 348835438, 62483237, 266980320,
                 151825527, 466206508, 288735205, 237104103, 437856897, 73540072, 229742722, 1003831, 83092282,
                 264678534, 271971130, 204650483, 394918211, 321857016, 290348383, 436873727, 362249359, 372172128] + [
                    370133522, 298663873, 383390264, 329148988, 441462736, 199376584, 257527578, 166527623, 142937186,
                    464646604, 118327550, 234994474, 260004324, 183985250, 349095149, 139285832, 360156606, 200723869,
                    320004517, 179034327, 158025009, 333473672, 349576261, 470381900, 218795833,
                    408636441, 76923707, 353475866, 202426247, 387690507, 209464063, 12421862, 296739893, 350618622,
                    407126408, 55650590, 335630746, 55525572, 342642208, 394357918] + [52005579, 219850915, 39699648, 232976128, 393818343, 198241702, 154872375, 224297258, 158002130, 198356533, 391949880, 232982558, 306955329, 237232044, 219857012, 417931607, 289580577, 268334473, 21535395, 287080092, 142381532, 9348006, 286916251, 445903569, 8967242, 441765914, 166648874, 142387023, 201177276, 91987762, 1167538, 159781361, 417931300, 389900760, 391903064, 8348911, 153065527, 382602147, 268532343, 22233480, 120826158]
    ...
    # ax.plot(mass, r, c=palette[0], zorder=4, label='Water world')
    ax.plot(mass, rho, c=palette[0], zorder=4, label='Water world', linewidth=2)
    plt.text(mass[-1]-22, rho[-1]-0.13, 'Water world', color=palette[0], fontweight='bold', fontsize=10, ha='center',
             va='center', zorder=4, rotation=23)
    ### rocky core + H/He atmos ###
    mass = np.linspace(1, 30, 100)
    r, rho, rho1, rho2 = rogers_2023_rocky_core(mass)
    # ax.plot(mass, r, c=sns.color_palette('muted')[5], zorder=4, label='Rocky core with H/He atmosphere')
    ax.plot(mass, rho, c=sns.color_palette('muted')[5], zorder=4, label='Rocky core with H/He atmosphere', linewidth=2)
    ax.plot(mass, rho1, c=sns.color_palette('muted')[5], zorder=4, ls='--')
    ax.plot(mass, rho2, c=sns.color_palette('muted')[5], zorder=4, ls='--')
    plt.text(mass[0]+0.95, rho[0]+0.015, 'Rocky+atmosphere', color=sns.color_palette('muted')[5], fontweight='bold', fontsize=10, ha='center',
    ...
    ### solar planets ###
    # plt.scatter(0.0553,0.985,facecolors='none', edgecolors='k', zorder=4, marker='o')
    # plt.text(0.0553/1.1, 0.985, 'Mercury', color='k', fontsize=10, ha='right', va='center', zorder=4, rotation=0)
    # plt.scatter(0.815,0.951,facecolors='none', edgecolors='k', zorder=4, marker='o')
    # plt.text(0.815/1.05, 0.951, 'Venus', color='k', fontsize=10, ha='right', va='center', zorder=4, rotation=0)
    plt.scatter(1, 1, facecolors='none', edgecolors='k', zorder=4, marker='o')
    plt.text(1/1.05, 1, 'Earth', color='k', fontsize=10, ha='right', va='center', zorder=4, rotation=0)
    # plt.scatter(0.107,0.714,facecolors='none', edgecolors='k', zorder=4, marker='o')
    # plt.text(0.107, 0.714, 'Mars', color='k', fontsize=10, ha='right', va='center', zorder=4, rotation=0)
    plt.scatter(95.2,0.125,facecolors='none', edgecolors='k', zorder=4, marker='o')
    plt.text(95.2/1.05, 0.125, 'Saturn', color='k', fontsize=10, ha='right', va='center', zorder=4, rotation=0)
    # plt.scatter(14.5,0.230,facecolors='none', edgecolors='k', zorder=4, marker='o')
    # plt.text(14.5/1.05, 0.230, 'Uranus', color='k', fontsize=10, ha='right', va='center', zorder=4, rotation=0)
    plt.scatter(17.1,0.297,facecolors='none', edgecolors='k', zorder=4, marker='o')
    plt.text(17.1*1.05, 0.297, 'Neptune', color='k', fontsize=10, ha='left', va='center', zorder=4, rotation=0)
    plt.xscale('log')
    plt.ylim(0,2)
    plt.xlim(0.5,100)
    # plt.yscale('log')
...
def figure_10(folder='/Users/tehan/Documents/TGLC/', recalculate=False):
    t = ascii.read(pkg_resources.resource_stream(__name__, 'PSCompPars_2024.12.07_14.30.50.csv'))
    difference_tglc = ascii.read(f'{folder}deviation_TGLC_2024.dat')
    tics_fit = [int(tic_sec.split('_')[1]) for tic_sec in difference_tglc['Star_sector']]
    tics = [int(s[4:]) for s in t['tic_id']]
