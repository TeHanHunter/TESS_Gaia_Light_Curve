...
def ffi_to_source(sector=1, camera=1, local_directory=''):
    ...
    with Pool() as p:
      p.map(partial(cut_ffi, camera=camera, sector=sector, size=150, path=local_directory), [1, 2, 3, 4])


if __name__ == '__main__':
    sector = 1
    for i in range(4):
        ffi_to_source(sector=sector, camera=i, local_directory=f'/home/tehan/data/sector{sector:04d}/')
